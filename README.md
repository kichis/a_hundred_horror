...
- 投稿やアカウントの削除が選択された時、削除処理をする前にユーザに再確認する、というプロセス(削除対象の重要度によってダイアログ(alert)とモーダルの２通り)
...
　: これまでmasterブランチから直接プッシュしていた。
   ...
   作業用ブランチで作業中に別種の不具合を見つけた時に、別のブランチを作成して対応&pushするなど、実践を通してなんとなくの感覚が掴めたのは良かった。
- 実装機能ごとにpushする。


## 実装できなかったこと(しなかったこと)
- 削除・編集の処理完了後、「〇〇を削除しました」というコメントを表示した方がユーザにとってわかりやすかった。(処理後、所定のページに遷移するだけだと不安を感じる)
